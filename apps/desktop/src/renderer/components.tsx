...
import { X,Folder,Image as ImageIcon,Film,LoaderCircle,Check,Star,MoreHorizontal,Eye,Pin,Tag,Pencil,FolderInput,Trash2,Copy,ExternalLink,FolderOpen,ImagePlus } from 'lucide-react'
...
export type EntryAction='open'|'preview'|'favorite'|'tags'|'cover'|'locate'|'rename'|'move'|'trash'|'copy'|'reveal'|'external'
export function EntryMenu({entry,onAction,contextOpen=false,closeContext}:{entry:Entry;onAction:(action:EntryAction,entry:Entry)=>void;contextOpen?:boolean;closeContext?:()=>void}){const [open,setOpen]=useState(false);return <Dropdown.Root open={open||contextOpen} onOpenChange={value=>{setOpen(value);if(!value)closeContext?.()}}><Dropdown.Trigger asChild><button className="card-more icon-button" title="更多操作" aria-label={entry.name+' 的更多操作'} onClick={e=>e.stopPropagation()}><MoreHorizontal size={17}/></button></Dropdown.Trigger><Dropdown.Portal><Dropdown.Content className="context-menu" sideOffset={4} onClick={e=>e.stopPropagation()}>
 {([['open',entry.kind==='folder'?'进入文件夹':'打开',FolderOpen],['preview','快速预览',Eye],['favorite',entry.favorite?'取消收藏':'添加收藏',Star],['tags','编辑标签',Tag],...(entry.kind!=='image'?[['cover','设置封面',ImagePlus]]:[]),['locate','定位所在目录',Pin],['rename','重命名',Pencil],['move','移动到…',FolderInput],['trash','移入回收站',Trash2],['copy','复制路径',Copy],['reveal','在资源管理器中显示',Folder],['external','使用默认应用打开',ExternalLink]] as [EntryAction,string,typeof Folder][]).map(([action,label,Icon])=><Dropdown.Item key={action} className={action==='trash'?'danger-text':''} onSelect={()=>onAction(action,entry)}><Icon size={15}/>{label}</Dropdown.Item>)}
