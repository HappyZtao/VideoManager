...
const initial={query:defaultQuery,selected:[],snapshot:null,snapshotCount:0,anchor:0,anchorId:null,history:[],future:[],views:{}}
export const persistentBrowse=(s:Browse)=>({...snapshot(s),selected:s.selected.slice(0,1000),snapshot:null,snapshotCount:0})
function safeBrowse(value:unknown):Browse|null{if(!value||typeof value!=='object')return null;const s=value as Browse;const q=querySchema.safeParse(s.query);if(!q.success)return null;return {query:q.data,selected:Array.isArray(s.selected)?s.selected.filter(id=>typeof id==='string').slice(0,1000):[],snapshot:null,snapshotCount:0,anchor:Number.isInteger(s.anchor)&&s.anchor>=0?s.anchor:0,anchorId:typeof s.anchorId==='string'?s.anchorId:null}}
