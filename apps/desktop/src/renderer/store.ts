...
export const defaultQuery:QuerySpec={folderId:null,scope:'library',text:'',searchFields:['name','folder','tag'],kinds:[],extensions:[],tags:[],favorite:false,actress:'',sort:'name',direction:'asc',movies:false,minSize:null,maxSize:null,after:null,before:null,minDuration:null,maxDuration:null}
export type ViewMode='library'|'movies'|'actresses'
...
type UIState=Browse&{views:Record<string,Browse>;history:Browse[];future:Browse[];hydrate:(views:unknown,last:unknown)=>void;navigate:(id:string|null,scope?:QuerySpec['scope'],patch?:Partial<QuerySpec>)=>void;openMovies:()=>void;openActresses:()=>void;setQuery:(patch:Partial<QuerySpec>)=>void;select:(ids:string[],snapshot?:string|null,count?:number)=>void;back:()=>void;forward:()=>void;setAnchor:(index:number,id:string|null)=>void;reset:()=>void}
...
function safeBrowse(value:unknown):Browse|null{if(!value||typeof value!=='object')return null;const s=value as Browse;const q=querySchema.safeParse(s.query);if(!q.success)return null;const view:ViewMode=s.view==='actresses'||s.view==='movies'?s.view:'library';return {view,query:q.data,selected:Array.isArray(s.selected)?s.selected.filter(id=>typeof id==='string').slice(0,1000):[],snapshot:null,snapshotCount:0,anchor:Number.isInteger(s.anchor)&&s.anchor>=0?s.anchor:0,anchorId:typeof s.anchorId==='string'?s.anchorId:null}}
export const useNavigation=create<UIState>((set,get)=>({...initial,
 ...
 openMovies:()=>{const old=get();if(old.view==='movies')return;const views={...old.views,[old.query.folderId??'library']:persistentBrowse(old)};set({view:'movies',history:[...old.history.slice(-49),snapshot(old)],future:[],views:Object.fromEntries(Object.entries(views).slice(-50)),query:{...defaultQuery},selected:[],snapshot:null,snapshotCount:0,anchor:0,anchorId:null})},
