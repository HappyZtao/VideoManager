...
struct Decoder {
 ...
 std::shared_ptr<Picture> step(int direction){if(window.empty())return seek(0);if(direction>0){if(selected+1<int(window.size()))return window.at(++selected);auto p=next();if(!p)throw std::runtime_error("Already at last frame");append(p);selected=int(window.size())-1;return p;}if(selected>0)return window.at(--selected);
