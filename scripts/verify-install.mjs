...
const record=JSON.parse((await fs.readFile('test-results/install-smoke.json','utf8')).replace(/^\uFEFF/,''));const directory=path.resolve(record.Directory);const {version}=JSON.parse(await fs.readFile('package.json','utf8'))
